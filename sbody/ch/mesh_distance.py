...
from sbody.matlab import row, col
# from sbody.mesh_distance import mesh_distance
import sample2meshdist
from sbody.robustifiers import SignedSqrt
...
class ClampedSignedPtsToMesh(ch.Ch):
    ...
    terms = 'a_min', 'a_max', 'reference_f', 'reference_template_or_sampler'

    def on_changed(self, which):
        dist = PtsToMesh(
            sample_verts=self.sample_v, reference_verts=self.reference_v, reference_faces=self.reference_f,
            reference_template_or_sampler=self.reference_template_or_sampler, signed=True, normalize=False)
...
class MeshDistanceSquared(Ch):
    ...
    def direction(self):
        from sbody.ch.ch_vert_normals import VertNormals, TriNormals
